...
func ShowTrainingInfo(action int, trainingType string, duration, weight, height float64, lengthPool, countPool int) string {
	...
	case trainingType == "Плавание":
		distance := distance(action) //float64(lengthPool*countPool) / mInKm
...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	// ваш код здесь
	distanceInKm := (float64(action) * lenStep) / mInKm
	avgSpeedKmh := distanceInKm / duration
	caloriesBurned := (runningCaloriesMeanSpeedMultiplier * avgSpeedKmh * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	caloriesBurned := ((walkingCaloriesWeightMultiplier * weight) + ((math.Pow(avgSpeedMsec, 2) / (height / cmInM)) * walkingSpeedHeightMultiplier * weight)) * duration * minInH
	return caloriesBurned
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь
	avgSpeedKmh := swimmingMeanSpeed(lengthPool, countPool, duration)
	caloriesBurned := (avgSpeedKmh + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
	return caloriesBurned
}
